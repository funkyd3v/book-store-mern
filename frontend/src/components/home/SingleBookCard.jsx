...
const SingleBookCard = ({book}) => {
  return (
    <div
      key={book._id}
      className="border-2 border-gray-500 px-4 py-2 rounded-lg m-4 relative hover:shadow-xl"
    >
      <h2 className="absolute top-1 right-2 px-2 py-1 mt-2 bg-red-300 rounded-lg">
        {book.publishYear}
      </h2>
      ...
      <div className="flex justify-between gap-x-2 items-center">
        <Link to={`/books/details/${book._id}`}>
          <BsInfoCircle className="text-green-700 text-2xl hover:text-black" />
        </Link>
